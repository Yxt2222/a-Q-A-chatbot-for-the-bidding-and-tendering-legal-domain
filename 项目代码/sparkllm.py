...
class ChatSparkLLM(BaseChatModel):
    ...
    def lc_secrets(self) -> Dict[str, str]:
        return {
            ...
            "spark_patch_id": "IFLYTEK_SPARK_PATCH_ID",
        }
    ...
    spark_patch_id: Optional[str] = Field(default=None, alias="patch_id")
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["spark_patch_id"] = get_from_dict_or_env(
            values,
            ["spark_patch_id", "patch_id"],
            "IFLYTEK_SPARK_PATCH_ID",
             default=''
        )
        ...
        values["client"] = _SparkLLMClient(
            ...
            patch_id=values["spark_patch_id"],
        )
        return values
...
class _SparkLLMClient:
    ...
    def run(
        ...
        ws.patch_id = self.patch_id  # type: ignore[attr-defined]
    ...
    def on_open(self, ws: Any) -> None:
        ...
        data = json.dumps(
            self.gen_params(
                messages=ws.messages, user_id=ws.user_id, model_kwargs=ws.model_kwargs,patch_id=ws.patch_id
            )
        )
        ws.send(data)
    ...
    def gen_params(
        self, messages: list, user_id: str,patch_id=None,model_kwargs: Optional[dict] = None
    ) -> dict:
        data: Dict = {
            "header": {"app_id": self.app_id, "uid": user_id,"patch_id":[patch_id] if patch_id else None},
